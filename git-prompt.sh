        ...
        eval " $(
                git status 2>/dev/null |
                    sed -n '
                        ...
                        s/^# Your branch is ahead of .[/[:alnum:]]\+. by [[:digit:]]\+ commit.*/freshness=${WHITE}↑${dim}/p
                        s/^# Your branch is behind .[/[:alnum:]]\+. by [[:digit:]]\+ commit.*/freshness=${YELLOW}↓${dim}/p
                        s/^# Your branch and .[/[:alnum:]]\+. have diverged.*/freshness=${YELLOW}↕${dim}/p
...
parse_vcs_status() {
        ...
        echo -n "$cyan(${vcs_info}$vcs_color${file_list}$cyan)"
